...
from groq_config import get_groq_api_key, get_groq_model, groq_chat_completion
    ...
    with tab2:
            ...
            def get_medication_recommendation(disease_type, patient_data):
                if not get_groq_api_key():
                    return (
                        "Groq API key is not set. Add `GROQ_API_KEY` to a `.env` file next to `main.py`, "
                        "or add it to `.streamlit/secrets.toml`. Get a key at https://console.groq.com/keys"
                    )
                user_prompt = f"""
...
"""
                try:
                    ...
                    return (
                        "Groq rate limit was hit (HTTP 429). Wait and retry, or set `GROQ_MODEL` in `.env`. "
                        "See https://console.groq.com/docs/rate-limits"
                    )
                except APIStatusError as e:
                    return f"Groq API error ({get_groq_model()}): {e.message}"
                ...
                try:
                    ...
                    try:
                        ...
                        if disease_type != "No diabetes detected":
                            ...
                            # Call Groq to generate medication recommendations
                            medication_info = get_medication_recommendation(disease_type, patient_data)

                            st.info("AI recommended medication (Groq):")
